...
const ReservaSala = () => {
  ...
  const salas = ['Sala 101', 'Sala 102', 'Sala 103', 'Sala 201', 'Sala 202'];
  const horarios = [
  ...
  const verificarDisponibilidade = (sala, dataHora, duracao) => {
    const reservas = JSON.parse(localStorage.getItem('reservas') || '[]');
    const inicio = new Date(dataHora);
    const fim = new Date(inicio.getTime() + duracao * 60000);

    return !reservas.some(reserva => {
      if (reserva.recurso !== sala || reserva.status === 'cancelada') return false;
  ...
  const handleSubmit = (e) => {
    ...
    if (pessoas < 1 || pessoas > 5) {
      setErro('Número de pessoas deve ser entre 1 e 5');
      return;
    }

    const dataHora = `${data}T${hora}:00`;
    
    if (!verificarDisponibilidade(sala, dataHora, duracao)) {
      setErro('Horário não disponível para esta sala');
      return;
    }
  ...
  return (
    <div className="reserva-container">
      ...
      <div className="regras-reserva">
        <h4>Regras para Reserva de Salas:</h4>
        <ul>
          <li>Mínimo 1 pessoa, máximo 5 pessoas por sala</li>
          <li>Check-in obrigatório (até 5 min antes, tolerância de 15 min)</li>
          <li>Tempo máximo: 2 horas para técnico/graduação</li>
          <li>Limite de 2-3 reservas por dia</li>
        </ul>
      </div>
      ...
      <form onSubmit={handleSubmit} className="reserva-form">
        ...
        <button type="submit" className="btn-reservar">Reservar Sala</button>
      </form>
    </div>
  );
};
